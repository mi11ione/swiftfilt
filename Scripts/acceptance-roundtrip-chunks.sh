#!/bin/bash
# Checkpointed full-corpus roundtrip acceptance: 1M-row foreground chunks.
# Usage: run-roundtrip-chunks.sh <first-chunk> <last-chunk>
set -u
REPO=/Users/mi11ion/Desktop/swiftfilt
DIR="$REPO/.build/parity-reports/acceptance"
CK="$DIR/roundtrip-checkpoint.log"
...
for i in $(seq "$1" "$2"); do
  ...
  SWIFTFILT_DEMANGLE_CORPUS="$CORPUS" "$REPO/.build/release/swiftfilt-parity" \
    roundtrip --skip "$skip" --limit "$CHUNK" --tag "$tag" > "$DIR/roundtrip-$tag.log" 2>&1
  ec=$?
  processed=$(grep -o 'symbols processed: [0-9,]*' "$DIR/roundtrip-$tag.log" | head -1 | tr -d ',' | awk '{print $3}')
  gating=$(grep -o 'UNEXPLAINED DIVERGENCES: [0-9,]*' "$DIR/roundtrip-$tag.log" | head -1 | tr -d ',' | awk '{print $3}')
